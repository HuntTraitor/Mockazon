...
export class ProductService {
  ...
  async create(product: NewProduct, vendor_id?: UUID): Promise<Product> {
    console.log(`http://${process.env.MICROSERVICE_URL||'localhost'}:3011/api/v0/product?vendorId=${vendor_id}`)
    return new Promise((resolve, reject) => {
      fetch(`http://${process.env.MICROSERVICE_URL||'localhost'}:3011/api/v0/product?vendorId=${vendor_id}`, {
        method: "POST",
